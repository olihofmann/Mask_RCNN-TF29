...
############################################################
#  MaskRCNN TF2 Model Class
############################################################

class MaskRCNN_TF2(tf.keras.Model):
    def __init__(self, config, custom_metrics, **kwargs):
    ...
    def metrics(self):
        ...
        return [m for m in self.custom_metrics.values()]
    ...
    def train_step(self, data):
        ...
        # update metrics
        self.custom_metrics["loss"].update_state(combined_loss)
        self.custom_metrics["rpn_class_loss"].update_state(rpn_class_loss)
        self.custom_metrics["rpn_bbox_loss"].update_state(rpn_bbox_loss)
        self.custom_metrics["mrcnn_class_loss"].update_state(class_loss)
        self.custom_metrics["mrcnn_bbox_loss"].update_state(bbox_loss)
        self.custom_metrics["mrcnn_mask_loss"].update_state(mask_loss)

        return {name: m.result() for name, m in self.custom_metrics.items()}
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            outputs = [rpn_class_logits, rpn_bbox,
                       ...
                       mrcnn_class_logits, mrcnn_bbox, mrcnn_mask,
                       active_class_ids]
            model = MaskRCNN_TF2(config=config, custom_metrics=custom_metrics, inputs=inputs, outputs=outputs, name='mask_rcnn')
        ...
        if config.GPU_COUNT > 1:
            from src.mask_rcnn.nn.parallel_model import ParallelModel
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(self.checkpoint_path, verbose=0, save_weights_only=True, monitor="val_loss"),
        ]
